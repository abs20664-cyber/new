    ...
    return (
        ...
        >
            ...
            <div className="fixed bottom-4 inset-x-0 flex justify-center z-50 px-2 pointer-events-none">
                <nav className="bg-surface/90 dark:bg-institutional-900/90 backdrop-blur-2xl border border-institutional-200 dark:border-institutional-800 flex items-center p-1.5 px-3 shadow-[0_20px_50px_rgba(0,0,0,0.3)] rounded-[2rem] gap-1 pointer-events-auto max-w-[95vw] sm:max-w-full overflow-x-auto scroll-smooth scroll-hide snap-x no-scrollbar">
                    {routes.map((route) => {
                    ...
                    <div onClick={() => setIsLangOpen(!isLangOpen)} className="p-3 text-institutional-600 hover:text-primary transition-all relative cursor-pointer shrink-0 snap-center" role="button" tabIndex={0}>
                        <Languages size={18} />
                        {isLangOpen && (
                            <div className={`fixed bottom-24 left-1/2 -translate-x-1/2 w-48 bg-surface dark:bg-institutional-900 border border-institutional-200 dark:border-institutional-800 rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200 z-[60] p-2`}>
                                {languages.map(l => (
                                    <button 
                                        key={l.code} 
                    ...
                    <div onClick={() => { setIsNotifOpen(!isNotifOpen); if (!isNotifOpen) markAllRead(); }} className="p-3 text-institutional-600 hover:text-primary transition-all relative cursor-pointer shrink-0 snap-center" role="button" tabIndex={0}>
                        <div className="relative">
                            <Bell size={18} />
                            {unreadCount > 0 && !isNotifOpen && <div className="absolute -top-1 -right-1 w-2 h-2 bg-primary rounded-full animate-ping" />}
                        </div>
                        {isNotifOpen && (
                            <div className={`fixed bottom-24 left-1/2 -translate-x-1/2 w-64 bg-surface dark:bg-institutional-900 border border-institutional-200 dark:border-institutional-800 rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200 z-[60] p-4`}>
