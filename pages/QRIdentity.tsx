...
import { Copy, Check, ShieldCheck, QrCode as QrIcon, Fingerprint } from 'lucide-react';
    ...
    return (
        <div className="min-h-full flex flex-col items-center justify-center p-4 md:p-8 fade-in">
            <div className="w-full max-w-lg academic-card p-10 md:p-14 flex flex-col items-center text-center relative overflow-hidden">
                ...
                <div className="absolute top-0 inset-x-0 h-1 bg-institutional-100 dark:bg-institutional-800 overflow-hidden">
                ...
                <div className="w-full mb-10 flex flex-col items-center">
                    <div className="w-full flex justify-between items-center mb-10">
                        <div className="flex items-center gap-2">
                            ...
                            <div className="text-start">
                                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-text leading-none">Status</p>
                                <p className="text-[9px] font-black uppercase tracking-[0.1em] text-success mt-1">Authenticated</p>
                            </div>
                        </div>
                        <div className="flex flex-col items-end">
                            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-text-secondary/50">Security Lev. 04</span>
                            <div className="flex gap-0.5 mt-1.5">
                                {[1,2,3,4].map(i => <div key={i} className="w-4 h-1 bg-primary rounded-full"></div>)}
                                <div className="w-4 h-1 bg-institutional-200 dark:bg-institutional-700 rounded-full"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="relative inline-block mb-6 group">
                        ...
                        <div className="absolute -bottom-2 -right-2 bg-primary text-white p-2.5 rounded-2xl border-4 border-sidebar shadow-xl shadow-primary/30 transform rotate-12 transition-transform hover:rotate-0">
                            <Fingerprint size={18} />
                        </div>
                    </div>

                    <h2 className="text-3xl font-black tracking-tight text-text mb-2 uppercase">
                        {user?.name}
                    </h2>
                ...
                <div className="relative group mb-12">
                    ...
                    <div className="mt-8 flex items-center justify-center gap-3 opacity-40 group-hover:opacity-80 transition-opacity">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.3em] text-text">{t('dashboard.scanPresence')}</span>
                    </div>
                </div>
                ...
                <div className="w-full space-y-5">
                    <div className="bg-institutional-50 dark:bg-institutional-800/40 p-6 rounded-3xl border border-institutional-200 dark:border-institutional-800 flex items-center justify-between group transition-all hover:border-primary/30">
                        <div className="text-start overflow-hidden">
                            <p className="text-[9px] font-black uppercase text-text-secondary/50 tracking-[0.2em] mb-1">{t('dashboard.uniqueId')}</p>
                            <p className="text-sm font-mono font-bold text-text truncate tracking-wider">
                                {user?.id}
                            </p>
                        </div>
                        <button 
                            onClick={handleCopyId}
                            className={`p-4 rounded-2xl transition-all active:scale-90 ${copied ? 'bg-success text-white shadow-lg shadow-success/20' : 'bg-sidebar text-text-secondary hover:text-primary shadow-sm hover:shadow-xl border border-border'}`}
                        >
                    ...
                    <div className="flex items-center justify-center gap-6">
                        ...
                        <div className="flex items-center gap-2.5">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-text-secondary/50">{t('dashboard.validSession')}</span>
                        </div>
